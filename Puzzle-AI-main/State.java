...
public class State {
    ...
    public State(int[] v, int size) // Khai báo trạng thái node
    {
        Value = v;
        Size = size;
        Length = Size * Size;
    ...
    public int[] StateArray() // Lấy mảng giá trị
    {
        return Value;
    }
    ...
    public int[] createArrayRandom() // Tạo mảng Random
    {
        Init();
    ...
    public int[] ArrayTronHinh() // Trộn hình
    {
        int t = 65;
        Init();
        this.blank = 0;
    ...
    public int estimate(State goalstate) // lựa chọn hàm heuristic
    {
        ...
        if (heuristic == 1)
            ...
            est = heuristic2(goalstate);
        else if (heuristic == 3)
            est = heuristic3(goalstate);
        return est;
    }

    public int heuristic1(State goalstate) // tổng khoảng cách dịch chuyển ngắn nhất để dịch chuyển các ô sai về vị trí
                                           // đúng của nó
    {
    ...
    public int heuristic2(State goalstate) // hàm h2 = heuristic1 + chỉ số phạt cặp ô hàng xóm với nhau đang nằm ngược
                                           // vị trí của nhau
    {
        int[] goal = goalstate.Value;
        int distance = 0;
        ...
        for (int i = 0; i < Length; i++) {
            ...
            // Tính chỉ số phạt
            if ((i != 0) && (i % Size != Size - 1) && (Value[i] == i + 1) && (Value[i + 1] == i))
                a += 2;
            if ((i != 0) && (i < Length - Size) && (Value[i] == i + Size) && (Value[i + Size] == i))
                a += 2;
        }
    ...
    public int heuristic3(State goalstate) // Hàm ước lượng đề xuất
    {
        int[] goal = goalstate.Value;
        int distance = 0;
        int a = 0;
