...
public class Solve {
    ...
    // private Vector<Node> OPEN; // tập trạng thái con của 1 tập bất kì
    ...
    public void solveBFS() { // Giải quyết bfs
        KQ.clear();
        long startTime = System.currentTimeMillis();
        // initialnode.g = 0;
        FRINGE.add(0, initialnode); // cho nút đầu đầu tiên vào FRINGE
        ...
        while (true) {
            ...
            n = FRINGE.elementAt(0); // xét nút n có fmin
            FRINGE.removeElement(n); // xóa nút đã xét trong FRINGE
            count++; // tăng số nút đã duyệt

            if (n.equals(goalnode)) // kiểm tra nút đang xét có phải là đích
            {
                long endTime = System.currentTimeMillis();
                time_solve = endTime - startTime;
                total_nodes = count + FRINGE.size();
    ...
    public void solveIDS() { // Giải quyết IDS
        KQ.clear();
        long startTime = System.currentTimeMillis();
        initialnode.f = initialnode.estimate(goalnode);
        initialnode.g = 0;
        FRINGE.add(0, initialnode); // cho nút đầu đầu tiên vào FRINGE
        cutOff = 0;
        total_nodes = 0;
        ...
        while (true) {
            while (FRINGE.size() != 0) // quá trình tìm kiếm sâu dần
            {
                ...
                if (n.equals(goalnode)) // kiểm tra n là đích chưa
                {
                    count++;
                    long endTime = System.currentTimeMillis();
                    time_solve = endTime - startTime;
            ...
            cutOff++;// đặt ngưỡng cutOff là fmin
            for (int i = 0; i < VISIT.size(); i++)
                FRINGE.add(0, VISIT.elementAt(i));
            VISIT.clear();
        }
